        ...
        <el-button size="default" type="primary" class="ml10" @click="onOpenSearch()">
          ...
          查询
			...
			<el-table :data="state.tableData.data" v-loading="state.tableData.loading" style="width: 100%">
				...
				<el-table-column prop="name" label="生肖" show-overflow-tooltip></el-table-column>
...
import {useZodiacApi} from "/src/api/zodiac";
// 引入组件
const Dialog = defineAsyncComponent(() => import('/src/views/server/zodiac/dialog.vue'));
...
const getTableData = () => {
  ...
  useZodiacApi().getZodiacList(state.tableData.param).then((res:any)=>{
    if (res.code == 200 ) {
...
const onRowDel = (row: any) => {
	ElMessageBox.confirm(`此操作将永久删除角色名称：“${row.name}”，是否继续?`, '提示', {
		...
		.then(() => {
      useZodiacApi().ZodiacDel({'id':row.id}).then((res:any)=>{
        if (res.code == 200 ) {
          getTableData();
...
const OpenStatus = (row:any) =>{
  useZodiacApi().ZodiacStatus(row).then((res:any)=>{
    if (res.code != 200 ) {
