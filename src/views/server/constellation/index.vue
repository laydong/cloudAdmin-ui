	...
	<div class="system-role-container layout-padding">
		<div class="system-role-padding layout-padding-auto layout-padding-view">
			<div class="system-user-search mb15">
				<el-input v-model="state.tableData.param.name" size="default" placeholder="请输入星座名" style="max-width: 200px"> </el-input>
        ...
        <el-button size="default" type="primary" class="ml10" @click="onOpenSearch()">
          ...
          查询
			...
			<el-table :data="state.tableData.data" v-loading="state.tableData.loading" style="width: 100%">
				...
				<el-table-column prop="name" label="星座" show-overflow-tooltip></el-table-column>
        <el-table-column prop="icon" label="图标" show-overflow-tooltip>
					<template #default="scope">
						<el-avatar shape="square" :src="scope.row.icon" />
					</template>
				</el-table-column>
        <el-table-column prop="start_date" label="日期" show-overflow-tooltip>
					<template #default="scope">
						<span>{{onTime(scope.row.start_date,scope.row.end_date)}}</span>
					</template>
				</el-table-column>
...
import {useConstellationApi} from "/src/api/constellation";
// 引入组件
const Dialog = defineAsyncComponent(() => import('/src/views/server/constellation/dialog.vue'));
...
const getTableData = () => {
	...
	useConstellationApi().getConstellationList(state.tableData.param).then((res:any)=>{
    if (res.code == 200 ) {
...
const onRowDel = (row: any) => {
	ElMessageBox.confirm(`此操作将永久删除角色名称：“${row.name}”，是否继续?`, '提示', {
		...
		.then(() => {
      useConstellationApi().ConstellationDel({'id':row.id}).then((res:any)=>{
        if (res.code == 200 ) {
          getTableData();
...
const OpenStatus = (row:any) =>{
  useConstellationApi().ConstellationStatus(row).then((res:any)=>{
    if (res.code != 200 ) {
