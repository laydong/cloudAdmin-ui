...
export async function setFilterMenuAndCacheTagsViewRoutes() {
	...
	await storesRoutesList.setRoutesList(setBackEndHasHasMenu(dynamicRoutes[0].children));
...
export function setBackEndHasHasMenu(routes: any) {
	...
	if (userInfo.menu_info.length > 0)
	{
		routes.forEach((item: any) => {
			if(userInfo.menu_info.includes(item.name)){
